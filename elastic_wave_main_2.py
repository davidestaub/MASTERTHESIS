...
if len( sys.argv ) > 1:
    ...
    if model_index == 0:
        ...
        model_type ="Simple_PlaneWave_Pinns"
    elif model_index == 10:
        model_type = "Simple_Bessel_Pinss"
    elif model_index == 11:
        model_type = "Global_FullDomain_Conditioned_Pinns_batched"
    else:
...
if (not (config['Network']['nn_type'] == 'FCN_ALL_PARAMS_WAVELET_FCN' or config['Network']['nn_type'] == 'FCN_ALL_PARAMS_PLANEWAVE_FCN' )):
    name = '{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}'.format(
        model_type,
        ...
        config['Network']['nn_type'],
        config['Network']['n_1'],
        config['parameters']['model_type'],
        config['parameters']['sigma_quake']
    )
else:
    name = '{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}'.format(
        model_type,
        config['description']['tag'],
        config['Network']['n_points'],
        config['optimizer']['n_epochs'],
...
wandb.login(relogin=True,key='143cbc4dedd46b8d5c70d6873dc8a909e85e066d')
...
if len( sys.argv ) > 1:
    ...
    if model_index == 0:
        ...
        pinn = PINNs.Simple_PlaneWave_Pinns(int(config['Network']['n_points']),wandb_on=True,config=config)
        print("model type selected as Simple_PlaneWave_Pinns")
        config_for_run.set('Network', 'model_type', 'Simple_PlaneWave_Pinns')
    elif model_index == 10:
        pinn = PINNs.Simple_Bessel_Pinns(int(config['Network']['n_points']),wandb_on=True,config=config)
        print("model type selected as Simple_Bessel_Pinss")
        config_for_run.set('Network', 'model_type', 'Simple_Bessel_Pinss')

    elif model_index ==11:
        pinn = PINNs.Global_FullDomain_Conditioned_Pinns_batched(int(config['Network']['n_points']),wandb_on=True,config=config)
        print("model type selected as Global_FullDomain_Conditioned_Pinns_batched")
        config_for_run.set('Network', 'model_type', 'Global_FullDomain_Conditioned_Pinns_batched')
    else:
        raise Exception("Unsupported model index {} please use a number between 0 and 6 : \n 0 -- PINNs \n 1 -- Global_NSources_Conditioned_Pinns \n 2 -- Relative_Distance_NSources_Conditioned_Pinns \n 3 -- Relative_Distance_FullDomain_Conditioned_Pinns \n 4 -- Global_NSources_Conditioned_Lame_Pinns \n 5 -- Global_FullDomain_Conditioned_Pinns \n -- Global_FullDomain_Conditioned_Pinns_Scramble_Resample ".format(model_index))
    ...
    if model_type == "Pinns":
        ...
        pinn = PINNs.PlaneWave_Pinns(int(config['Network']['n_points']),wandb_on=True,config=config)
    elif model_type =="Simple_PlaneWave_Pinns":
        pinn = PINNs.Simple_PlaneWave_Pinns(int(config['Network']['n_points']),wandb_on=True,config=config)
    elif model_type == "Simple_Bessel_Pinss":
        pinn = PINNs.Simple_Bessel_Pinns(int(config['Network']['n_points']),wandb_on=True,config=config)
    elif model_type == "Global_FullDomain_Conditioned_Pinns_batched":
        pinn = PINNs.Global_FullDomain_Conditioned_Pinns_batched(int(config['Network']['n_points']), wandb_on=True,config=config)
    else:
...
#pinn = PINNs.Advanced_Bessel_Pinns(int(config['Network']['n_points']),wandb_on=True,config=config)

pinn = PINNs.Hard_data_PINNs(int(config['Network']['n_points']), wandb_on=True,config=config)
...
optimizer_ADAM = optim.Adam(pinn.approximate_solution.parameters(),lr=1e-3,weight_decay=1e-3)
...
#pretrained_pth = 'Global_FullDomain_Conditioned_Pinns_FCN_ALL_PARAMS_PLANEWAVE_FCN_500000_300_1.0_200_200_100_4_64_tanh_0.07_FCN_ALL_PARAMS_PLANEWAVE_FCN_128_3_16_mixture/model.pth'
#helper_network = PINNs.Pinns(int(config['Network']['n_points']), wandb_on=True,config=config)
#pinn.approximate_solution.load_state_dict(torch.load(pretrained_pth, map_location=torch.device('cpu')))
#pinn.approximate_solution.params_layer.weight.data = helper_network.approximate_solution.params_layer.weight.data.clone()
#pinn.approximate_solution.params_layer.bias.data = helper_network.approximate_solution.params_layer.bias.data.clone()

#pinn.approximate_solution.load_state_dict(helper_network.approximate_solution.state_dict())
#print(pinn.approximate_solution.params_layer.weight.data )
...
if config['accoustic']['accoustic_on'] =='True':
    hist = pinn.fit_accoustic(n_epochs, optimizer_LBFGS, verbose=True)
else:
