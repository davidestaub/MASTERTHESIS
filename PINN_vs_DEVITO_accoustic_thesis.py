...
pth1 = '../pre_trained_models/accoustic/Global_FullDomain_Conditioned_Pinns_accoustic_conditioned_1000000_400_1.0_200_200_100_4_64_tanh_0.2_FCN_ALL_PARAMS_PLANEWAVE_FCN_135_3_16_mixture_0.1'

pth1 = '../pre_trained_models/accoustic/Global_FullDomain_Conditioned_Pinns_accoustic_conditioned_1000000_400_1.0_200_200_100_4_64_tanh_0.2_FCN_ALL_PARAMS_PLANEWAVE_FCN_135_3_16_mixture_0.1'
pth2 = '../pre_trained_models/accoustic/Global_FullDomain_Conditioned_Pinns_accoustic_1000000_400_1.0_200_200_100_7_128_tanh_0.2_PINN_1_mixture_0.1'



new_folder = [pth1,pth2]
...
for input_folder in new_folder:
    ...
    save_path = '../../Results/Accoustic/test/[{}_{}]/{}/'.format( str(mu_quake[0]), str(mu_quake[1]),str_path)
    ...
    f = plt.figure(figsize=(7.9, 4.9))  # Adjust figure size as needed
    f.subplots_adjust(left=.1, right=.9)
    main_gs = gridspec.GridSpec(1, n_columns, hspace=0.1, wspace=0)  # Adjust hspace for spacing between groups

    # Set row titles for the leftmost column
    titles = [r"PINN $\phi$", r"FD $\phi$", r"$\triangle \phi$"]
    ...
    for idx, (h, time_value) in enumerate(zip(time_steps_to_plot, time_values_to_plot)):
        ...
        # Create a subgrid for this single group in the current column (time step)
        sub_gs = gridspec.GridSpecFromSubplotSpec(3, 1, subplot_spec=main_gs[0, idx], hspace=0.0, wspace=0.0)
        ...
        for row_idx in range(3):
            ...
            # Select data to plot based on the row
            if row_idx == 0:  # PINN u
                data = res[h, :, :]
            elif row_idx == 1:  # FD u
                data = res_list_devito[h]
            else:  # Difference
                data = res[h, :, :] - res_list_devito[h]
            ...
            if idx == 0:
                ax.text(-0.1, 0.5, titles[row_idx], transform=ax.transAxes, fontsize=15, va='center', ha='right',
                        rotation=0)

            # Add the rectangle with L2 error text in the bottom row of each column
            if row_idx == 2:
                # Determine the size of the rectangle
                rect_width, rect_height = 0.3, 0.3  # Adjust these values as needed
    ...
    f.tight_layout()
    f.savefig(save_path + "selected_time_steps_comparison.png",dpi=1000)
    f.show()
    ...
    print(len(res_list))
    print("Average relative l2 error:", (total_relative_l2_error / len(res_list)) * 100.0, "%", " dt = ", dt, "n = ",
          n)
