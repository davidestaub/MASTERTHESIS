...
def initial_condition_explosion_conditioned(input_tensor, sigma):
    ...
    #TODO: Very ugly workaround for plot:
    if True:
        # Normalize gradients to get initial velocity field
        u0x = -grad_x / 6.0653
        u0y = -grad_y / 6.0653
        ...
        # Normalize gradients to get initial velocity field
        u0x = -grad_x / torch.max(torch.abs(grad_x)+1e-20)
        u0y = -grad_y / torch.max(torch.abs(grad_y)+1e-20)
...
def initial_condition_simple_gaussian_accoustic(input_tensor,sigma):
    # Create grid
    #x_part = torch.pow(input_tensor[:, 1] - mu_quake[0], 2)
    #y_part = torch.pow(input_tensor[:, 2] - mu_quake[1], 2)

    # Shifted Gaussian centered at mu_quake
    #gauss = torch.exp(- (x_part + y_part) / (2 * sigma ** 2))

    # Normalize the Gaussian
    #u0 = gauss / torch.max(gauss)
    ...
    u0 = torch.exp(exponent)
    return u0

def initial_condition_simple_gaussian_accoustic_conditioned(input_tensor,sigma):
    x = input_tensor[:, 1] - input_tensor[:, 3]
    y = input_tensor[:, 2] - input_tensor[:, 4]

    x_part = torch.pow(x, 2)
    y_part = torch.pow(y, 2)
...
if __name__ == "__main__":
    ...
    def initial_condition_explosion(input_tensor, sigma, mu_quake):
        x = input_tensor[:, 0] - mu_quake[0]
        y = input_tensor[:, 1] - mu_quake[1]
        gauss = torch.exp(- (x ** 2 + y ** 2) / (2 * sigma ** 2))
        grad_x = -x * gauss / (sigma ** 2)
        grad_y = -y * gauss / (sigma ** 2)
        u0x = grad_x / (torch.max(torch.abs(grad_x))+1e-20)
        u0y = grad_y / (torch.max(torch.abs(grad_y))+1e-20)
        return u0x, u0y
    ...
    # Compute u0x, u0y
    u0x, u0y = initial_condition_explosion(input_tensor, sigma, mu_quake)
