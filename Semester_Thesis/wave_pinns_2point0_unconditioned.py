    ...
    def get_layered_velocity(self, input):
        ...
        for (index, element) in enumerate(out):
            ...
            if (y_value < -0.4333):
                ...
                out[index] = lerp(y_value, -0.4333, -0.2333, 2.943, 1.323)

            elif (y_value < 0.2333):
                out[index] = 1.323

            elif (y_value > 0.2333 and y_value < 0.4333):
                out[index] = lerp(y_value, 0.2333, 0.4333, 1.323, 2.943)
...
wandb.init(project='test',name = 'report 3 layer unconditioned unnormalized showuld match prev')
...
torch.save(pinn.approximate_solution.state_dict(),
           "report_unconditioned_unnormalized_3layers_to_match_experiment.pth")
