...
class Pinns:
    ...
    def compute_loss(self, inp_train_tb, u_train_tb, inp_train_tb2, u_train_tb2, inp_train_int,velocity, verbose=True):
        ...
        r_tb = u_train_tb - self.apply_initial_condition(inp_train_tb)
        ...
        r_int = self.compute_pde_residual1(inp_train_int,velocity)
    ...
    def fit(self, num_epochs, optimizer, verbose=False):
        ...
        vel_select_list = [0.5,1.0,2.0,3.0,4.0]
        vel_soboleng = torch.quasirandom.SobolEngine(dimension=4)
        vel = vel_soboleng.draw(self.n_tb)
        vel =  vel * (vel_select_list[-1] - vel_select_list[0]) + vel_select_list[0]
        print(vel)
...
wandb.init(project='test',name='sobo 50k')
