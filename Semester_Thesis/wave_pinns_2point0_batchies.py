...
class Pinns:
    def __init__(self, n_int_, n_tb_, n_tb_upsample_):
        ...
        self.domain_extrema = torch.tensor([[0.0, 0.5],  # Time dimension
        ...
        self.approximate_solution = NeuralNet(input_dimension=7, output_dimension=1,
                                              n_hidden_layers=7,
                                              neurons=128,
    ...
    def get_layered_velocity(self, input):
        ...
        for (index, element) in enumerate(out):
            ...
            width_half = 0.1
            boundary1A = (start + (abs(start) + abs(end))/4) - width_half
            boundary1B = (start + (abs(start) + abs(end))/4) + width_half

            boundary2A = (start + 2.0 *(abs(start) + abs(end)) / 4) - width_half
            boundary2B = (start + 2.0 *(abs(start) + abs(end)) / 4) + width_half

            boundary3A = (start + 3.0 * (abs(start) + abs(end)) / 4) - width_half
            boundary3B = (start + 3.0 * (abs(start) + abs(end)) / 4) + width_half

            y_value = input[index, 2]
            #if (y_value < -0.4333):
                #out[index] = input[index,3]

            #elif (y_value > -0.4333 and y_value < -0.2333):
               # out[index] = lerp(y_value, -0.4333, -0.2333, input[index,3], input[index,4])

            #elif (y_value < 0.2333):
                #out[index] = input[index,4]

            #elif (y_value > 0.2333 and y_value < 0.4333):
               # out[index] = lerp(y_value, 0.2333, 0.4333, input[index,4], input[index,5])

            #else:
               # out[index] = input[index,5]

            if (y_value < boundary1A):
                out[index] = input[index, 3]

            elif (y_value > boundary1A and y_value < boundary1B):
                out[index] = lerp(y_value, boundary1A, boundary1B, input[index, 3], input[index, 4])

            elif (y_value < boundary2A):
                out[index] = input[index, 4]

            elif (y_value > boundary2A and y_value < boundary2B):
                out[index] = lerp(y_value, boundary2A, boundary2B, input[index, 4], input[index, 5])

            elif (y_value < boundary3A):
                out[index] = input[index, 5]

            elif (y_value > boundary3A and y_value < boundary3B):
                out[index] = lerp(y_value, boundary3A, boundary3B, input[index, 5], input[index, 6])

            else:
                out[index] = input[index, 6]
    ...
    def compute_pde_residual1(self, input_int,velocity):
        ...
        residual1 = dx_gradient[:, 1] + dy_gradient[:, 2] - (1 / (torch.pow(velocity, 2))) * dt_gradient[:, 0]
        residual = residual1
    ...
    def compute_loss(self, inp_train_tb, u_train_tb, inp_train_tb2, u_train_tb2, inp_train_int,velocity, verbose=True):
        ...
        loss_tb = torch.mean(abs(u_train_tb - self.apply_initial_condition(inp_train_tb)) ** 2)
        ...
        loss = torch.log10(2 * (loss_u) + loss_int)
    ...
    def fit(self, num_epochs, optimizer, verbose=False):
        ...
        vel_select_list = [1.0,2.0,3.0,4.0]
        n_vel_configs = len(vel_select_list)**4
        print(inp_train_int.shape)
        vel = torch.full(((inp_train_int.shape[0],4)), 1.0)
        variations = []
        for element in itertools.product(vel_select_list,repeat=4):
            variations.append(element)


        for i in range(n_vel_configs):
            start = int(i * int(vel.shape[0] / n_vel_configs))
            end = int(start + int(vel.shape[0] / n_vel_configs))
        ...
        inp_train_int = torch.column_stack((inp_train_int, vel))
        inp_train_tb = torch.column_stack((inp_train_tb, vel))
        inp_train_tb2 = torch.column_stack((inp_train_tb2, vel))
        ...
        #generat random indexes to load on gpu
        indices = torch.empty((num_epochs,inp_train_tb.size()[0]))
        for epoch in range(num_epochs):
            randindex = torch.randperm(inp_train_tb.size()[0])
            indices[epoch,:] = randindex
        ...
        for epoch in range(num_epochs):
            ...
            for i in range(0,4):
                ...
                start = i * int(inp_train_tb.size()[0]/4)
                end = start + int(inp_train_tb.size()[0]/4)
                ...
                def closure():
                    ...
                    if epoch <= (only_init_percentage / 100) * num_epochs:
                        loss = self.compute_loss_init(inp_train_tb[indices[epoch]][start:end], u_train_tb[indices[epoch]][start:end], inp_train_tb2[indices[epoch]][start:end], u_train_tb2[indices[epoch]][start:end],verbose=verbose)
                    else:
                        loss = self.compute_loss(inp_train_tb[indices[epoch]][start:end], u_train_tb[indices[epoch]][start:end], inp_train_tb2[indices[epoch]][start:end], u_train_tb2[indices[epoch]][start:end], inp_train_int[indices[epoch]][start:end],velocity[indices[epoch]][start:end],verbose=verbose)
                    loss.backward()
...
wandb.init(project='test',name='80k 2 batches test')
...
torch.save(pinn.approximate_solution.state_dict(),
           "80k_two_batches_variational_conditined_from_1to4_4layers_lambda_25nr2_epochs_4k_lr_05_full_batch_domain-1t01_unnormalized.pth")
