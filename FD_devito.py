...
def generate_acoustic_mixture_np(num_mixtures=100,amplitude=0.3, width=0.15):
    np.random.seed(42)
...
def initial_condition_simple_gaussian(sigma, mu_quake, nx, ny, dx, dy):
    # Create grid
    ...
    # Shifted Gaussian centered at mu_quake
    gauss = np.exp(-0.5* (np.sqrt((X - mu_quake[0]) ** 2 + (Y - mu_quake[1]) ** 2) / sigma ) **2)
    print("Position of Gaussian maximum:", np.unravel_index(np.argmax(gauss), gauss.shape))

    # Normalize the Gaussian
    u0 = gauss / np.max(gauss)
    print("max init value =", np.max(u0))
