...
mu_quake_x_list = np.linspace(-0.3,0.3,20)
mu_quake_y_list = np.linspace(-0.3,0.3,20)
...
mu_quake_x_list = [0.265909]
mu_quake_y_list = [-0.061364]
    ...
    for mu_quake_y in mu_quake_y_list:
        ...
        for test_model in ['Vanilla','PLANEWAVE']:
            ...
            if test_model == 'Vanilla':
                save_path = 'Results/Elastic/conditioned_thesis/[{}_{}]/Vanilla/'.format(str(mu_quake_x),str(mu_quake_y))
                os.makedirs(save_path, exist_ok=True)
                input_folder = '../pre_trained_models/'+pth2
            else:
                save_path = 'Results/Elastic/conditioned_thesis/[{}_{}]/WAVELET/'.format(str(mu_quake_x),str(mu_quake_y))
                os.makedirs(save_path, exist_ok=True)
                input_folder = '../pre_trained_models/'+pth1
            ...
            for idx, (h, time_value) in enumerate(zip(time_steps_to_plot, time_values_to_plot)):
                ...
                for diff in [diffx, diffy, diffu]:
                    diff[0:10, :] = 0
                    diff[:, 0:10] = 0
                    diff[:, numpoints_sqrt - 10:numpoints_sqrt] = 0
                    diff[numpoints_sqrt - 10:numpoints_sqrt:, :] = 0

                for group_idx in range(3):
                    # Create a subgrid for each group in the current column (time step)
                    sub_gs = gridspec.GridSpecFromSubplotSpec(3, 1, subplot_spec=main_gs[group_idx, idx], hspace=0.0,wspace=0.0)
