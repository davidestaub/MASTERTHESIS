...
#TODO add n_different_sources to config file
...
class Pinns:
    ...
    def pinn_model_eval(self, input_tensor,source_function):
        ...
        u0x, u0y = source_function(input_tensor)
    ...
    def add_solid_points(self):
        ...
        if config['Network']['conditioning'] == 'True':
            n_different_sources = config['initial_condition']['n_sources']
    ...
    def compute_solid_loss(self, input_s,lambda_m,mu_m,source_function):
        U = self.pinn_model_eval(input_s,source_function)
        u_x = U[:, 0].unsqueeze(1)
        u_y = U[:, 1].unsqueeze(1)
    ...
    def get_solid_residual(self, input_s,lambda_m,mu_m,source_function):
        U = self.pinn_model_eval(input_s,source_function)
        u_x = U[:, 0].unsqueeze(1)
        u_y = U[:, 1].unsqueeze(1)
        gradient_x = torch.autograd.grad(u_x.sum(), input_s, create_graph=True)[0]
    ...
    def compute_loss(self, inp_train_s,lambda_m,mu_m,source_function):
        loss_solid = self.compute_solid_loss(inp_train_s,lambda_m,mu_m,source_function)
    ...
    def fit(self, num_epochs, optimizer, verbose=False):
        ...
        if config['parameters']['model_type'] == 'mixture':
        ...
        elif config['parameters']['model_type'] == 'constant':
            lambda_m = torch.full((self.n_collocation_points,), float(config['parameters']['lambda_solid']))
            mu_m = torch.full((self.n_collocation_points,), float(config['parameters']['mu_solid']))
            print(lambda_m.shape,mu_m.shape)
        else:
            raise Exception("{} not implemented".format(config['parameters']['model_type']))
        ...
        if config['initial_condition']['source_function'] == 'explosion':
            source_function = initial_conditions.initial_condition_explosion
        elif config['initial_condition']['source_function'] == 'explosion_conditioned':
            source_function = initial_conditions.initial_condition_explosion_conditioned
        elif config['initial_condition']['source_function'] == 'explosion_two_sources':
            source_function = initial_conditions.initial_condition_explosion_two_sources
        elif config['initial_condition']['source_function'] == 'gaussian':
            source_function = initial_conditions.initial_condition_gaussian
        elif config['initial_condition']['source_function'] == 'donut':
            source_function = initial_conditions.initial_condition_donut

        elif config['initial_condition']['source_function'] == 'fancy':
            source_function = initial_conditions.initial_condition_explosion_conditioned_fancy
        else:
        ...
        for epoch in range(num_epochs):
            if config['visualize']['visualize_on'] == 'True':
                ...
                for i in time_list:
                    ...
                    plot_input[:, 0] = i
                    residual, U_with_ansatz, U_withouth_ansatz = self.get_solid_residual(plot_input,lambda_m,mu_m,source_function)
            ...
            def closure():
                optimizer.zero_grad()
                loss = self.compute_loss(training_set_s,lambda_m,mu_m,source_function)
                loss.backward()
