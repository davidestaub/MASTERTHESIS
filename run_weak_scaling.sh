...
SIGMA=$9
N_POINTS=${10}  # New variable for n_points

# Copy the original configuration file to a new file that includes the parameter values
CONFIG_FILE="config_n1_${N_1}_neurons_${N_NEURONS}_hidden_${N_HIDDEN_LAYERS}_hiddenafter_${N_HIDDEN_LAYERS_AFTER}_neuronsafter_${N_NEURONS_AFTER}_type_${NN_TYPE}_tag_${TAG}_t1_${T1}_sigma${SIGMA}_npoints${N_POINTS}.ini"
...
sed -i "s/^[[:space:]]*n_points = .*$/n_points = ${N_POINTS}/" $CONFIG_FILE  # Add this line for n_points

# Run the python script with the new configuration file
CUDA_LAUNCH_BLOCKING=1 python elastic_wave_main_2.py $CONFIG_FILE 0
